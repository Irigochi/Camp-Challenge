...
<html>
    ...
    <form action="insertconfirm" method="POST">
        名前:
        <input type="text" name="name" value="<% if(reinput){out.print(udb.getName());}%>" required="">
        <br><br>
        ...
        <br>
            ...
            <input type="radio" name="type" value="<%=i%>"<%
                if(reinput && udb.getType() == i){out.print("checked = \"checked\"");}%> required=""><%=jh.exTypenum(i)%><br>
            <% } %>
        <br>

        電話番号:
        <input type="text" name="tell" value="<% if(reinput){out.print(udb.getTell());}%>" required="">
        <br><br>

        自己紹介文
        <br>
        <textarea name="comment" rows=10 cols=50 style="resize:none" wrap="hard" required=""><%
            if(reinput){out.print(udb.getComment());}%></textarea><br><br>
